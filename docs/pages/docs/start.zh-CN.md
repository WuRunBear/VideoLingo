...
注意：VideoLingo 会把当前正在处理的视频中间文件放在 `static/output` 文件夹中，并在处理完成后自动移到 `static/history` 文件夹。如果你想强制结束，可以直接把 `static/output` 下的文件移动或删除。
...
1. **翻译过程的 'All array must be of the same length' 或 'Key Error'**: 
   ...
   解决方案：检查 `static/output/gpt_log/error.json` 的 `response` 和 `msg` 字段，删掉 `static/output/gpt_log` 文件夹后重试。
